  ...
  <page-header-wrapper>
    <a-card :bordered="false">
      ...
      <div class="table-operator">
        <a-button type="primary" icon="plus" @click="handleAdd()" v-if="hasPermission('role.create')">新建</a-button>
      </div>
      ...
      <s-table ref="table" size="default" rowKey="id" :columns="columns" :data="loadData" showPagination="auto">
        ...
        <span slot="action" slot-scope="text, record">
          <template>
            <a @click="handleEdit(record)" v-if="hasPermission('role.update')">编辑</a>
            ...
            <a-popconfirm
              ...
              v-if="record.name !== 'admin' && hasPermission('role.delete')"
            >
      ...
      >
        <a-form-model ref="form" :model="info" :rules="rules" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
          ...
          <a-form-model-item label="权限" prop="permissionIds" v-if="info.name !== 'admin'">
            <a-tree
              :checked-keys="permissionIds"
              checkable
              :check-strictly="true"
              :tree-data="permissionTree"
              @check="(keys) => (permissionIds = keys)"
            />
          </a-form-model-item>
        </a-form-model>
...
import { get, add, edit, del } from '@/api/role'
import { get as getPermissions } from '@/api/permission'
...
export default {
  ...
  mounted() {
    getPermissions({ includeChildren: true }).then((res) => {
  ...
  methods: {
    ...
    handleEdit(record) {
      ...
      this.permissionIds = record.permissions.map((x) => x.id)
      this.visible = true
    },
    handleOk() {
